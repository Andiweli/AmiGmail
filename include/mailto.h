#ifndef AMIGMAIL_MAILTO_H
#define AMIGMAIL_MAILTO_H
 ...
 * detached_child is set when the URI came from AmiGmail's private temporary
...
/* Start a detached copy of AmiGmail carrying url.  On non-Amiga hosts this
 * returns 0. */
int amg_mailto_spawn_detached(const char *url);

/* A lightweight public Exec port used only to hand mailto: requests to an
 * already running AmiGmail instance.  The sender transfers ownership of the
