    ...
    const ComposeAttachment *attachments, size_t count)
{
    ...
    pending->uid = uid;
    if (used) {
        pending->paths = (char (*)[COMPOSE_PATH_MAX])calloc(
            used, sizeof(*pending->paths));
        if (!pending->paths) {
            free(pending);
            return NULL;
        }
    }
    for (i = 0; i < count && pending->count < used; ++i) {
                                       ...
                                       size_t count)
{
    ...
    /* Network commands are completed FIFO. Keep one marker for every queued
     * compose send/save, even when it owns no temporary files. This preserves
     * ordering for commands that share type/UID (notably new mail/draft UID 0)
     * and prevents a response from consuming a later forward's cleanup. */
    pending->next = NULL;
                                       ...
                                       size_t length, AmgError *error)
{
    ...
    snprintf(attachment->path, sizeof(attachment->path),
             "T:AmiGmail-compose-%08lx-%08lx-%08lx-%04lx-%02lu.tmp",
                          ...
                          ComposeDraftSeed *draft_seed, AmgError *error)
{
    ...
    dialog = WindowObject,
        WA_Title, edit_draft
            ? T(MSG_AMIGMAIL_EDIT_DRAFT, "AmiGmail - Edit draft")
            : (reply_mode
                ? T(MSG_AMIGMAIL_REPLY, "AmiGmail - Reply")
                : (forward_mode
                    ? T(MSG_AMIGMAIL_FORWARD, "AmiGmail - Forward")
                    : T(MSG_AMIGMAIL_NEW_MAIL, "AmiGmail - New mail"))),
    ...
    if (queued_action == 1)
        status_local(gui, edit_draft
            ? T(MSG_SENDING_DRAFT, "Sending draft...")
            : T(MSG_SENDING_MAIL, "Sending mail..."));
