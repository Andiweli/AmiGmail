...
#define AMG_MAILTO_IPC_PORT "AMIGMAIL.MAILTO"
#define AMG_MAILTO_IPC_MAGIC 0x414D474DUL /* 'AMGM' */
#define AMG_MAILTO_IPC_MAX 32768U
#define AMG_MAILTO_FILE_PREFIX "--amg-mailto-file="
#define AMG_MAILTO_TEMP_PREFIX "T:AmiGmailMailto."
                            ...
                            AmgError *error)
{
    ...
    *output = NULL;
    ...
    if (!decoded) {
        amg_error_set(error, AMG_ERR_MEMORY, "Not enough memory for mailto URL.");
    ...
    for (in_pos = 0U; in_pos < length; ++in_pos) {
        ...
        if (c == '%') {
            ...
            if (in_pos + 2U >= length) {
                free(decoded);
                amg_error_set(error, AMG_ERR_PARSE, "Invalid percent escape in mailto URL.");
            ...
            if (c == 0U) {
                free(decoded);
                amg_error_set(error, AMG_ERR_PARSE, "NUL byte is not allowed in mailto URL.");
                return AMG_ERR_PARSE;
            }
        }
                            ...
                            AmgError *error)
{
    ...
    if (!*destination || !**destination) {
        ...
        *destination = duplicate_text(value);
        if (!*destination) {
            amg_error_set(error, AMG_ERR_MEMORY, "Not enough memory for mailto recipient.");
            return AMG_ERR_MEMORY;
        }
        return AMG_OK;
    }
    ...
    if (old_length > (size_t)-1 - value_length - 3U) {
        amg_error_set(error, AMG_ERR_LIMIT, "mailto recipient list is too long.");
        return AMG_ERR_LIMIT;
    }
    combined = (char *)malloc(old_length + value_length + 3U);
    if (!combined) {
        amg_error_set(error, AMG_ERR_MEMORY, "Not enough memory for mailto recipients.");
        return AMG_ERR_MEMORY;
    }
                     ...
                     AmgError *error)
{
    ...
    if (!url || !request || !ascii_prefix_nocase(url, AMG_MAILTO_SCHEME)) {
        amg_error_set(error, AMG_ERR_ARGUMENT, "Expected a mailto: URL.");
        return AMG_ERR_ARGUMENT;
    }
    if (strlen(url) > AMG_MAILTO_IPC_MAX) {
        amg_error_set(error, AMG_ERR_LIMIT, "mailto: URL is too long.");
        return AMG_ERR_LIMIT;
    }
...
static char *load_mailto_temp_file(const char *path, AmgError *error)
{
    ...
    if (fseek(file, 0, SEEK_END) != 0 || (size = ftell(file)) < 0 ||
        ...
        fclose(file);
        amg_error_set(error, AMG_ERR_IO, "Could not read mailto hand-off file.");
        return NULL;
    }
    ...
    if (!ascii_prefix_nocase(url, AMG_MAILTO_SCHEME)) {
        free(url);
        amg_error_set(error, AMG_ERR_PARSE, "Invalid mailto hand-off file.");
        return NULL;
    }
    (void)remove(path);
    return url;
                             ...
                             int *detached_child, AmgError *error)
{
    ...
    if (found) {
        ...
        if (!path) {
            amg_error_set(error, AMG_ERR_MEMORY,
                          "Not enough memory for mailto hand-off path.");
            return NULL;
        }
    ...
    if (found) {
        ...
        if (!copy)
            amg_error_set(error, AMG_ERR_MEMORY,
                          "Not enough memory for mailto URL.");
        return copy;
    }
