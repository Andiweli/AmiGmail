...
int message_list_uses_recipient(const AmgGui *gui, size_t label_index)
{
    return gui && label_index < gui->label_count &&
           (gui->labels[label_index].special_use & AMG_LABEL_SENT) != 0UL;
}
...
void set_message_party_column_mode(AmgGui *gui, int recipient)
{
    ...
    SetLBColumnInfoAttrs(gui->columns,
                         ...
                         TAG_DONE);
    if (gui->messages_gadget && gui->window) {
                                           ...
                                           int *parse_error)
{
    ...
    while (length > 0U &&
        ...
        char party[513], subject[769], date[160];
        struct Node *node;
        ...
        party_header = amg_mail_header_get(&headers, recipient ? "To" : "From");
        if (recipient && (!party_header || !*party_header))
            party_header = amg_mail_header_get(&headers, "Cc");
        if (recipient && (!party_header || !*party_header))
            party_header = amg_mail_header_get(&headers, "Bcc");
