...
#define AMG_UPDATE_API_URL \
    "https://api.github.com/repos/Andiweli/AmiGmail/releases/latest"
#define AMG_UPDATE_DOWNLOAD_PREFIX \
    "https://github.com/Andiweli/AmiGmail/releases/download/"
...
static int parse_version_parts(const char *text,
                               unsigned long *parts, size_t capacity,
                               size_t *count)
{
    const char *cursor;
    size_t used = 0;
    if (!text || !*text || !parts || !capacity || !count) return 0;
    cursor = text;
    if (*cursor == 'v' || *cursor == 'V') ++cursor;
    if (!isdigit((unsigned char)*cursor)) return 0;
    while (*cursor) {
        unsigned long value = 0UL;
        if (used >= capacity || !isdigit((unsigned char)*cursor)) return 0;
...
{
    unsigned long candidate[8], current[8];
    size_t candidate_count = 0U, current_count = 0U, i, max_count;
    memset(candidate, 0, sizeof(candidate));
    memset(current, 0, sizeof(current));
    if (!parse_version_parts(candidate_tag, candidate,
                             sizeof(candidate) / sizeof(candidate[0]),
                             &candidate_count) ||
        !parse_version_parts(current_version, current,
                             sizeof(current) / sizeof(current[0]),
                             &current_count))
        return 0;
    max_count = candidate_count > current_count
        ? candidate_count : current_count;
    for (i = 0; i < max_count; ++i) {
        unsigned long a = i < candidate_count ? candidate[i] : 0UL;
        unsigned long b = i < current_count ? current[i] : 0UL;
        if (a > b) return 1;
        if (a < b) return 0;
    }
    return 0;
...
{
    unsigned long version_parts[8];
    size_t version_count = 0U;
    int written;
    if (!json || !info) return AMG_ERR_ARGUMENT;
    memset(info, 0, sizeof(*info));
    memset(version_parts, 0, sizeof(version_parts));
    if (!json_extract_string(json, length, "tag_name",
                             info->tag, sizeof(info->tag)) ||
        !parse_version_parts(info->tag, version_parts,
                             sizeof(version_parts) / sizeof(version_parts[0]),
                             &version_count)) {
    ...
    written = snprintf(info->download_url, sizeof(info->download_url),
                       AMG_UPDATE_DOWNLOAD_PREFIX "%s/AmiGmail-%s.lha",
                       info->tag, info->tag);
...
int amg_update_check_latest(AmgUpdateInfo *info, AmgError *error)
{
    ...
    amg_error_set(error, AMG_ERR_UNSUPPORTED,
                  "Update check is available only on AmigaOS.");
                        ...
                        AmgError *error)
{
    ...
    (void)destination;
    amg_error_set(error, AMG_ERR_UNSUPPORTED,
                  "Update download is available only on AmigaOS.");
    return AMG_ERR_UNSUPPORTED;
}
