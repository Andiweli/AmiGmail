...
static int send_new_mail(AmgNetwork *network, AmgNetMessage *message,
                         const char *access_token, AmgError *error)
{
    ...
    return amg_smtp_send_mail(&network->account, access_token, &draft, error);
}
                      ...
                      AmgError *error)
{
    ...
    network->process = CreateNewProcTags(
        ...
        NP_Name, (ULONG)"AmiGmail network",
