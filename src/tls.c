                                  ...
                                  unsigned long timeout_seconds, AmgError *error)
{
    ...
    if (!connection) { amg_error_set(error, AMG_ERR_MEMORY, T("Nicht genug Speicher.", "Not enough memory.")); return NULL; }
    connection->socket_fd = -1;
    connection->socket_fd = open_socket(host, port, timeout_seconds, error);
    if (connection->socket_fd < 0) goto fail;
    connection->context = SSL_CTX_new(TLS_client_method());
    if (!connection->context) { ssl_error(error, AMG_ERR_TLS, T("TLS-Kontext", "TLS context")); goto fail; }
    SSL_CTX_set_mode(connection->context, SSL_MODE_AUTO_RETRY);
    SSL_CTX_set_verify(connection->context, SSL_VERIFY_PEER, NULL);
    if (SSL_CTX_load_verify_locations(connection->context, "AmiSSL:Certs/ca-bundle.crt", NULL) != 1 &&
        SSL_CTX_set_default_verify_paths(connection->context) != 1) {
        ssl_error(error, AMG_ERR_TLS, T("CA-Zertifikate konnten nicht geladen werden", "CA certificates could not be loaded")); goto fail;
    }
    connection->ssl = SSL_new(connection->context);
    if (!connection->ssl) { ssl_error(error, AMG_ERR_TLS, T("TLS-Sitzung", "TLS session")); goto fail; }
    if (SSL_set_tlsext_host_name(connection->ssl, host) != 1 || SSL_set1_host(connection->ssl, host) != 1) {
        ssl_error(error, AMG_ERR_TLS, "TLS-Hostname"); goto fail;
    }
    SSL_set_fd(connection->ssl, connection->socket_fd);
    if (SSL_connect(connection->ssl) != 1) { ssl_error(error, AMG_ERR_TLS, "TLS-Handshake"); goto fail; }
    if (SSL_get_verify_result(connection->ssl) != X509_V_OK) {
        amg_error_set(error, AMG_ERR_TLS, T("Das Serverzertifikat ist ungültig. Bitte auch Datum und Uhrzeit prüfen.", "The server certificate is invalid. Please also check date and time.")); goto fail;
    }
    return connection;
fail:
    amg_tls_close(connection); return NULL;
}
                               ...
                               AmgError *error)
{
    ...
    if (ready == 0)
        amg_error_set(error, AMG_ERR_IO,
                      T("Zeit\303\274berschreitung beim Warten auf Gmail-Daten.", "Timed out waiting for Gmail data."));
    else
        amg_error_set(error, AMG_ERR_IO,
                      T("Socket-Wartefehler beim Gmail-Zugriff.", "Socket wait error while accessing Gmail."));
    return 0;
}
...
long amg_tls_read(AmgTlsConnection *connection, void *data, size_t length, AmgError *error)
{
    ...
    if (!connection || !connection->ssl || !data || !length) return AMG_ERR_ARGUMENT;
    for (;;) {
        ERR_clear_error();
        ...
        if (ssl_result == SSL_ERROR_ZERO_RETURN)
            amg_error_set(error, AMG_ERR_IO,
                          T("Gmail hat die TLS-Verbindung geschlossen.", "Gmail closed the TLS connection."));
        else
            ssl_error(error, AMG_ERR_IO, T("TLS-Lesefehler", "TLS read error"));
        return AMG_ERR_IO;
    }
}
...
long amg_tls_write(AmgTlsConnection *connection, const void *data, size_t length, AmgError *error)
{
    int result;
    if (!connection || !connection->ssl || (!data && length)) return AMG_ERR_ARGUMENT;
    result = SSL_write(connection->ssl, data, length > 0x7fffffffU ? 0x7fffffff : (int)length);
    if (result <= 0) { ssl_error(error, AMG_ERR_IO, T("TLS-Schreibfehler", "TLS write error")); return AMG_ERR_IO; }
    return result;
}
