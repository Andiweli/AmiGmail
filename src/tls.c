                                  ...
                                  unsigned long timeout_seconds, AmgError *error)
{
    ...
    if (!connection) { amg_error_set(error, AMG_ERR_MEMORY, T(MSG_NOT_ENOUGH_MEMORY, "Not enough memory.")); return NULL; }
    ...
    if (!connection->context) { ssl_error(error, AMG_ERR_TLS, T(MSG_TLS_CONTEXT, "TLS context")); goto fail; }
    ...
    if (SSL_CTX_load_verify_locations(connection->context, "AmiSSL:Certs/ca-bundle.crt", NULL) != 1 &&
        ...
        ssl_error(error, AMG_ERR_TLS, T(MSG_CA_CERTIFICATES_COULD_NOT_BE_LOADED, "CA certificates could not be loaded")); goto fail;
    }
    connection->ssl = SSL_new(connection->context);
    if (!connection->ssl) { ssl_error(error, AMG_ERR_TLS, T(MSG_TLS_SESSION, "TLS session")); goto fail; }
    ...
    if (SSL_get_verify_result(connection->ssl) != X509_V_OK) {
        amg_error_set(error, AMG_ERR_TLS, T(MSG_THE_SERVER_CERTIFICATE_IS_INVALID_PLEASE_ALSO_CHECK, "The server certificate is invalid. Please also check date and time.")); goto fail;
    }
    return connection;
fail:
                               ...
                               AmgError *error)
{
    ...
    if (ready == 0)
        amg_error_set(error, AMG_ERR_IO,
                      T(MSG_TIMED_OUT_WAITING_FOR_GMAIL_DATA, "Timed out waiting for Gmail data."));
    else
        amg_error_set(error, AMG_ERR_IO,
                      T(MSG_SOCKET_WAIT_ERROR_WHILE_ACCESSING_GMAIL, "Socket wait error while accessing Gmail."));
    return 0;
}
...
long amg_tls_read(AmgTlsConnection *connection, void *data, size_t length, AmgError *error)
{
    ...
    for (;;) {
        ...
        if (ssl_result == SSL_ERROR_ZERO_RETURN)
            amg_error_set(error, AMG_ERR_IO,
                          T(MSG_GMAIL_CLOSED_THE_TLS_CONNECTION, "Gmail closed the TLS connection."));
        else
            ssl_error(error, AMG_ERR_IO, T(MSG_TLS_READ_ERROR, "TLS read error"));
        return AMG_ERR_IO;
    }
}
...
long amg_tls_write(AmgTlsConnection *connection, const void *data, size_t length, AmgError *error)
{
    ...
    if (result <= 0) { ssl_error(error, AMG_ERR_IO, T(MSG_TLS_WRITE_ERROR, "TLS write error")); return AMG_ERR_IO; }
    return result;
}
