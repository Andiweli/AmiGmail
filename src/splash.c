...
static void splash_draw_banner(void)
{
    ...
    /* The text block below may be wider with another Workbench font.
     * Fill the complete banner slot first, so the #888888 band always
     * reaches the popup's content right edge instead of stopping at 170px. */
    if (slot->Width > 0 && slot->Height > 0) {
        SetAPen(rastport, (ULONG)splash_pens[0]);
        RectFill(rastport,
                 left, top,
...
static Object *splash_create_window_object(void)
{
    ...
    const char *client_line = "AmiGmail " AMIGMAIL_VERSION;
    const char *description_line =
        amg_tr(MSG_GMAIL_CLIENT_FOR_AMIGAOS_3_2, "Gmail client for AmigaOS 3.2");
    const char *copyright_line = "\251 Andreas St\374rmer";

    /* The class GetClass() entry points use the conventional global class
     * bases from the NDK proto headers. AmiGmail's main GUI has not opened
    ...
    root_group = NewObject(
        ...
        /* Minimum remains the native artwork width. Without a maximum the
         * slot expands to the width chosen by a wider text/font below. */
