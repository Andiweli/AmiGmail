...
static void signature_dialog(AmgGui *gui)
{
    ...
    editor = NULL;
    dialog = WindowObject,
        WA_Title, T(MSG_AMIGMAIL_SIGNATURE, "AmiGmail - Signature"),
...
static int request_label_index(AmgGui *gui, size_t index, AmgError *error)
{
    ...
    if (!gui->labels[index].available ||
        ...
        status_local(gui,
                     T(MSG_THIS_GMAIL_FOLDER_IS_NOT_AVAILABLE_FOR_THE, "This Gmail folder is not available for the account."));
    ...
    set_message_party_column_mode(gui,
                                  message_list_uses_recipient(gui, index));
...
static void delete_selected_messages(AmgGui *gui, AmgError *error)
{
    ...
    if (!source_label || !*source_label) {
        free(uids);
        status_local(gui, T(MSG_THE_CURRENT_GMAIL_FOLDER_IS_UNKNOWN, "The current Gmail folder is unknown."));
        return;
    }
...
static void empty_trash(AmgGui *gui, AmgError *error)
{
    ...
    if (!trash_label || !*trash_label) {
        status_local(gui, T(MSG_THE_GMAIL_TRASH_FOLDER_IS_UNKNOWN, "The Gmail Trash folder is unknown."));
        return;
    }
...
static void empty_spam(AmgGui *gui, AmgError *error)
{
    ...
    if (!spam_label || !*spam_label) {
        status_local(gui, T(MSG_THE_GMAIL_SPAM_FOLDER_IS_UNKNOWN, "The Gmail Spam folder is unknown."));
        return;
    }
        ...
        if (event.result != AMG_OK) {
            ...
            switch (event.type) {
                ...
                case AMG_NET_RECONFIGURE:
                    status_local(gui, T(MSG_CONNECTED_TO_GMAIL, "Connected to Gmail."));
                ...
                case AMG_NET_FETCH_LABELS:
                {
                    ...
                    amg_tr_snprintf(message, sizeof(message), MSG_VALUE_GMAIL_LABELS_LOADED, "%lu Gmail labels loaded.", (unsigned long)count);
                    status_local(gui, message);
                ...
                case AMG_NET_SEND_DRAFT:
                    if (current_folder_is_drafts(gui))
                        remove_message_uid(gui, event.uid);
                    status_local(gui, T(MSG_DRAFT_WAS_SENT, "Draft was sent."));
                    break;
...
void periodic_fetch_mail(AmgGui *gui, AmgError *error)
{
    ...
    if (!amg_network_is_connected(gui->network)) {
        status_local(gui, T(MSG_PERIODIC_FETCH_CONNECTING_TO_GMAIL, "Periodic fetch: connecting to Gmail..."));
...
void fetch_mail(AmgGui *gui, AmgError *error)
{
    ...
    if (result != AMG_OK) {
        ...
        status_local(gui, T(MSG_CONNECTING_TO_GMAIL, "Connecting to Gmail..."));
        result = amg_network_request(gui->network, AMG_NET_CONNECT, 0,
                                     NULL, NULL, error);
